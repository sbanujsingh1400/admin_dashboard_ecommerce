...
const Cart = () => {
  ...
  const [couponCode,setCouponCode]= useState<string>('a');
  ...
  return (
    <div className='cart flex justify-between items-center gap-[4rem] h-[calc(100vh-4rem)] px-[2rem] py-[4rem]'>
      <main className='w-[70%] overflow-y-auto hide-scrollbar  ' >
        {cartItem1 &&<CartItem cartItem={cartItem1} />}
        {!cartItem1&&<h1>No Items Added</h1>}
      </main>
      <aside className='w-[30%] p-[4rem] flex flex-col justify-center items-stretch gap-[1.5] ' >
        ...
        <input type="text" className='p-[0.2rem] focus:outline-none rounded-[5px] mt-[2rem]  border-[2px] border-solid border-black' value={couponCode} onChange={(e)=>{setCouponCode(e.target.value)}} placeholder='Coupon Code' />
          {couponCode && (isValidCouponCode?<span className='text-green-300 flex items-center justify-center gap-[5px] mt-[1rem] ' >₹{discount} off using the <code className='font-[900]  self-[flex-end] ' >{couponCode}</code></span>: <span className='text-red-600 flex items-center justify-center gap-[5px] -mt-[1rem]' >Invalid Coupon Code <VscError /></span>)}
